...
```{r}
storms = setcolorder(storms, c("name", "status","category"))
...
```{r}
storms$average_diameter = storms[ ,.(average_diameter = 
                                        ifelse(!(is.na(ts_diameter))&!(is.na(hu_diameter)), 
                                               mean(ts_diameter +hu_diameter), 
                                               ifelse(is.na(ts_diameter)&(is.na(hu_diameter)), NA ,
                                               ifelse(is.na(ts_diameter), hu_diameter,ts_diameter)))) ]
```
...
```{r}
storm_summary = storms[ ,.(maximum_wind = max(wind)), by = name]
...
```{r}
storms[ ,.(maximum_wind = max(wind)), by = c("name", "year", "month","day")][order(-year, -month,-day)]
...
```{r}
storms[ ,.(maximum_wind =  max(wind)) , by = c("year", "name")]
...
```{r}
storms[, .(maximum_category = max(category),maximum_wind = max(wind), maximum_pressure = max(pressure),maximum_diameter_ts = max(ts_diameter),maximum_diameter_hu = max(hu_diameter)), by = name]
...
```{r}
storms[ , .(max_wind_speed = max(wind)), by = c("year", "status") ]
...
```{r}
storms$average_location = storms[ ,.(average_location = long/lat)]
...
```{r}
storms[ , .(average_storm = mean(hour)), by = name]
...
library("lubridate")
storms[  , !"time"]
storms$timestamp = storms[ ,.(timestamp = ymd(paste(year , month, day, sep = "/")))]
```
...
```{r}
storms$day_of_week = storms[ ,.(day_of_week = wday(timestamp, label = TRUE))]
storms$week_of_year = storms[ ,.(week_of_year = week(timestamp))]
```

For each storm, summarize the day in which is started in the following format "Friday, June 27, 1975".

```{r}
storms[ , .(new_format = paste(wday(timestamp, label = TRUE,abbr = FALSE),month(timestamp, label = TRUE, abbr = FALSE), day ,year, sep = " , ")), by = name]
...
```{r}
storms$decile_windspeed = storms[, decile_windspeed = cut(wind, c(seq(1, max(wind), 10)))]
...
```{r}
serious_storms$average_location = serious_storms[ ,.(average_location = paste(long , lat, sep = "/"))]
...
```{r}
storms[ ,.( average_wind_speed = mean(wind), average_pressure = mean(pressure), average_ts_diameter = mean(ts_diameter), avverage_hu_diameter = mean(hu_diameter)), by = category]
...
```{r}
storms[ ,.( max_wind_speed = max(wind), max_pressure = max(pressure), max_ts_diameter = max(ts_diameter), max_hu_diameter = max(hu_diameter)), by = name]
...
distance_to_miami_funtion = function( MIAMI_COORDS, long, lat){
  long1 = MIAMI_COORDS[1]
  lat1 = MIAMI_COORDS[2]
  distance_to_miami_function = sqrt((long1 - long)^2+(lat1 - lat)^2)
}
storms$distance_to_miami = storms[ , .(distance_to_miami = distance_to_miami_funtion(MIAMI_COORDS, long, lat))]
```
...
```{r}
interaction_model = lm(medv ~ rm * tax, Boston)
vanilla_model = lm( medv ~ rm + tax, Boston)
summary(interaction_model)$r.squared
summary(vanilla_model)$r.squared
...
```{r}
ggplot(Boston, aes(x = rm, y = medv)) +
  geom_point(aes(color = crim , size = lstat))
```
...
```{r}
interaction_model = lm(medv ~ rm * crim*lstat, Boston)
vanilla_model = lm( medv ~ rm + crim + lstat, Boston)
summary(interaction_model)$r.squared
summary(vanilla_model)$r.squared
summary(interaction_model)$sigma
summary(interaction_model)$sigma
