...
Nres = get_tree_num_nodes_leaves_max_depths(tree_mod)
```

Plot the regression tree model with the optimal node size.

```{r}
x_predict = data.frame(x = seq(x_min, x_max, length.out = Nres))
g = predict(tree_mod, x_predict)
ggplot(data.frame(x = x, y = y), aes(x, y)) +
  geom_point(aes(x, y), data.frame(x = x_predict, y = g), col = "red")
```
