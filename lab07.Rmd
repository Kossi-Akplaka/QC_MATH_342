...
```{r}
# I did nt extrapolate when evaluating the models out of sample
...
```{r}
poly_mod_degree_2 = lm(medv ~ poly(crim, 2) + poly(zn, 2) + poly(indus, 2) + chas + poly(nox, 2) + poly(rm, 2) + poly(age, 2) + poly(dis, 2) + poly(rad, 2)  + poly(tax, 2) + poly(ptratio, 2) + poly(black, 2) + poly(lstat, 2), boston)
summary(poly_mod_degree_2)$sigma
summary(poly_mod_degree_2)$r.squared
```
...
```{r}
poly_mod_degree_3 = lm(medv ~ poly(crim, 3) + poly(zn, 3) + poly(indus, 3) + chas + poly(nox, 3) + poly(rm, 3) + poly(age,3 ) + poly(dis, 3) + poly(rad, 3)  + poly(tax, 3) + poly(ptratio, 3) + poly(black, 3) + poly(lstat, 3), boston)
summary(poly_mod_degree_3)$sigma
summary(poly_mod_degree_3)$r.squared

#This model did better than the degree 2 polynomial model
```
...
poly_reg_mod = lm(y~poly(x,9))
summary(poly_reg_mod)
```
...
ggplot(data.frame(x = x, y = y), aes(x ,y)) + geom_point() + geom_smooth(method = lm, formula = poly_reg_mod, col = "green")
...
ggplot(X, aes(age)) + 
  ...
  ggtitle("Age plot")
ggplot(X, aes(age)) +
  geom_histogram(binwidth = 1, col = "red", fill = "yellow", alpha = 0.4) +
  ggtitle("Age plot")
...
ggplot(X, aes(vocab)) + 
  geom_density(fill = "yellow", alpha = 0.4) +
  ggtitle("vocabulary plot")
ggplot(X, aes(vocab)) +
  geom_histogram(binwidth = .5, col = "red", fill = "yellow", alpha = 0.4) +
  ggtitle("vocabulary plot")
...
ggplot(X, aes(x = educGroup, y = vocab)) +
  geom_jitter(alpha = 0.1)+
  ggtitle("Vocubulary by Education level")
ggplot(X, aes(x = educGroup, y = vocab)) +
  geom_point() +
...
```{r}
ggplot(X, aes(x = educGroup, y = gender)) +
  geom_jitter(alpha = 0.1)+
  ggtitle("gender by education level")
```
...
angle <- c()
for (i in 1:(n-1)){
  theata_angle <- acos((X[ , i] %*% X[ , i+1]) / (sqrt(sum(X[ , i]^2)) * sqrt(sum(X[ , i+1]^2))))
  angle <- c(angle, theta-angle * 180 / pi)
}
angle
}
```
...
```{r}
ggplot() +
  geom_abline(log(fibr), col = "red") + geom_abline(log(fibcpp), col = "yellow")
```
